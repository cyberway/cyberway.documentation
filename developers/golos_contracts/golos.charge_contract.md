...
Обращение к батарейкам происходит через вызовы операций-действий. В смарт-контракте `golos.charge` реализованы встраиваемая (англ. inline) функция [get_stored](#vstraivaemaya-funkciya-get_stored) и операции-действия [use](#operaciya-deistvie-use), [useandstore](#operaciya-deistvie-useandstore), [removestored](#operaciya-deistvie-removestored), [setrestorer](#operaciya-deistvie-setrestorer), [usenotifygt](#operacii-deistvya-usenotifygt-i-usenotifylt), [usenotifylt](#operacii-deistviya-usenotifygt-i-usenotifylt)
...
Операция-действие `usenotifylt` используется для извещения (нотификации) смарт-контракта `golos.publication` о расходе ресурсов батарейки меньше установленного порогового значения на выполнение операций пользователя.
