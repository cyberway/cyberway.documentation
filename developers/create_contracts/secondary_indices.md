...
## 6 Вторичные индексы
В этом разделе приведено описание сортировки таблиц по индексам на примере структуры `voteinfo`, а также последовательность действий для добавления индекса в смарт-контракт `addressbook` для операций с записями в таблице.  
...
Описание таблицы вида `vote_table` должно соответствовать следующему формату:   
```
...
```
**6.6 Тестирование**
**6.6.1** Вставить записи в таблицу пользователей `alice` и `bob`.
```
...
```
**6.6.2** Проверить данные таблицы для пользователей `alice` и `bob` по индексу `age`.  В командных строках использовать параметр `--index 2` для указания запроса по вторичному индексу (индекс № 2).
```
...
**6.6.3** Поиск данных таблицы по вторичному индексу считается успешно выполненным, если будет получена информация вида:
